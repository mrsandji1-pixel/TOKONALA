...
async function cetakLabelLangsung(barcode) {
  ...
  localStorage.setItem('lastLabelSettings', JSON.stringify({
    w: wMM, h: hMM, g: gapMM, ox: oxMM, oy: oyMM, c: cols, q: qty, m: model
  }));
  ...
  try {
    ...
    if (typeof updateSettings === 'function') {
      await updateSettings({
        label_width: document.getElementById('labelWidthMM').value,
        label_height: document.getElementById('labelHeightMM').value,
        label_gap: document.getElementById('labelGapMM').value,
        paper_cols: document.getElementById('labelCols').value,
    ...
    for (var copy = 0; copy < printCount; copy++) {
      ...
      } else if (model === 'PUTIAN80L') {
        cmd = getPutian80LCommand(totalW, h, gap, ox, oy, cols, nama, harga, barcodeText, showNama, showHarga, showBarcode);
      } else if (model === 'PUTIAN80DL') {
        cmd = getPutian80DLCommand(totalW, h, gap, ox, oy, cols, nama, harga, barcodeText, showNama, showHarga, showBarcode);
      } else {
