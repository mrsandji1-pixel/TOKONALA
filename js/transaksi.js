...
// FIXED: renderPaymentButtons dengan fallback untuk metode tanpa handler
function renderPaymentButtons() {
  ...
  if (typeof paymentSettings !== 'undefined' && paymentSettings.methods) {
    var activeMethods = paymentSettings.methods.filter(function(m) { return m.aktif; });
    ...
    if (activeMethods.length === 0) {
      ...
      activeMethods.forEach(function(m) {
        ...
        var handlerFunc = 'bukaPopup' + btnId;
        
        // FIXED: Cek apakah handler ada, jika tidak gunakan handler tunai
        if (typeof window[handlerFunc] === 'function') {
          html += '<button class="btn btn-tunai" onclick="' + handlerFunc + '()">' + m.nama + '</button>';
        } else if (m.nama === 'TUNAI') {
          html += '<button class="btn btn-tunai" onclick="bukaPopupTUNAI()">TUNAI</button>';
        } else if (m.nama === 'QRIS') {
          html += '<button class="btn btn-tunai" onclick="bukaPopupQRIS()">QRIS</button>';
        } else {
          // FIXED: Untuk metode lain, gunakan handler tunai dengan label metode tersebut
          html += '<button class="btn btn-tunai" onclick="bukaPopupPembayaranLain(\'' + m.nama + '\')">' + m.nama + '</button>';
        }
      });
    }
  } else {
...
// FIXED: Added generic payment handler for custom methods
function bukaPopupPembayaranLain(namaMetode) {
  var modal = document.createElement('div');
  modal.id = 'popupPembayaranLainModal';
  ...
  html += '<h3>💳 Pembayaran ' + namaMetode + '</h3>';
  ...
  html += '<button class="btn-sm btn-danger" onclick="document.getElementById(\'popupPembayaranLainModal\').remove()" style="flex:1;">Batal</button>';
  html += '</div></div>';
...
function simpanTunai() {
  ...
  if (modal) modal.remove();
  var modal2 = document.getElementById('popupPembayaranLainModal');
...
async function bayarDanCetak() {
  ...
  try {
    ...
    if (typeof tampilkanPopupShare === 'function') {
...
async function tampilkanPesananTersimpan() {
  ...
  if (!orders.length) { listEl.innerHTML = '<p>Tidak ada pesanan</p>'; }
    ...
    orders.forEach(function(o) {
      ...
      html += '<br><button class="btn-sm" onclick="muatPesanan(\'' + o.no_pesanan + '\')">📥 Muat</button>';
      html += ' <button class="btn-sm btn-danger" onclick="hapusPesanan(\'' + o.no_pesanan + '\')">🗑</button></div>';
    });
