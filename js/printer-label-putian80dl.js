// ===================== PRINTER LABEL - Putian 80DL (TSPL) =====================
function getPutian80DLCommand(totalW, h, gap, ox, oy, cols, nama, harga, barcodeText, showNama, showHarga, showBarcode) {
  var cmd = '';
  
  cmd += 'SIZE ' + Math.round(totalW / 8) + ' mm,' + Math.round(h / 8) + ' mm\r\n';
  cmd += 'GAP ' + Math.round(gap / 8) + ' mm,0\r\n';
  cmd += 'DENSITY 10\r\n';
  cmd += 'DIRECTION 0\r\n';
  cmd += 'REFERENCE 0,0\r\n';
  cmd += 'CLS\r\n';
  ...
  for (var col = 0; col < cols; col++) {
    ...
    if (showBarcode && showHarga) {
      ...
      cmd += 'TEXT ' + (x + 125) + ',60,"3",0,1.3,1.3,"' + harga + '"\r\n';
    } else if (showBarcode) {
      cmd += 'BARCODE ' + x + ',50,"128",32,0,0,1,2,"' + barcodeText + '"\r\n';
    } else if (showHarga) {
