...
function bukaFitur(key) {
  ...
  if (typeof window[setupFuncName] === 'function') {
    ...
    if (typeof window[altFuncName] === 'function') {
      ...
      document.getElementById(contentId).innerHTML = '<p style="color:#999;">Fitur ini belum tersedia.</p>';
    }
  }
}
...
function updateLabelButtonVisibility() {
  ...
  if (typeof refreshProductList === 'function' && typeof activeTab !== 'undefined' && activeTab === 'inventory') {
    refreshProductList();
  }
}
...
async function setupModal_utang(containerId) {
  ...
  try {
    ...
    container.innerHTML = '<p>❌ Gagal memuat: ' + e.message + '</p>';
  }
}
